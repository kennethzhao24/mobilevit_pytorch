...
from lib.layers import norm_layers_tuple
...
def load_pretrained_model(model, wt_loc, is_master_node = False):
    if not os.path.isfile(wt_loc):
        raise ValueError('Pretrained file is not found here: {}'.format(wt_loc))
    ...
    if is_master_node:
        print('Pretrained weights are loaded from {}'.format(wt_loc))
    return model
